...
void f1_setup_response(const f1ap_setup_resp_t *resp)
{
  ...
  *mac->f1_config.setup_resp = cp_f1ap_setup_response(resp);
  ...
  for (int i = 0; i < resp->num_cells_to_activate; i++) {
    const served_cells_to_activate_t *cu_cell = &resp->cells_to_activate[i];
    // find the matching DU cell by cellid
    f1ap_served_cell_info_t *du_cell = NULL;
    for (int j = 0; j < sr->num_cells_available; j++) {
      if (sr->cell[j].info.nr_cellid == cu_cell->nr_cellid) {
        du_cell = &sr->cell[j].info;
        break;
      }
    }
    AssertFatal(du_cell != NULL, "CU activated cell %lu not found in DU setup request\n", cu_cell->nr_cellid);
    AssertFatal(check_plmn_identity(&du_cell->plmn, &cu_cell->plmn), "PLMN mismatch for cell %lu\n", cu_cell->nr_cellid);
    ...
    prepare_du_configuration_update(mac, du_cell, mib, sib1);
  }
  NR_SCHED_UNLOCK(&mac->sched_lock);
