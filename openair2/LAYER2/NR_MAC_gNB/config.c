...
static void config_common(nr_cell_sched_t *cell, const nr_mac_config_t *config, NR_ServingCellConfigCommon_t *scc, int cell_idx)
{
...
void nr_mac_config_scc(gNB_MAC_INST *nrmac, nr_cell_sched_t *cell, NR_ServingCellConfigCommon_t *scc, const nr_mac_config_t *config)
{
  ...
  for (int i = 0; i < num_beams; i++) {
    ...
    AssertFatal(cell->common_channels.vrb_map_UL[i],
                "could not allocate memory for RC.nrmac[]->cells[].common_channels.vrb_map_UL[%d]\n", i);
  }
  ...
  const int cell_idx = (int)(cell - nrmac->cells);
  AssertFatal(cell_idx >= 0 && cell_idx < NR_MAX_CELLS, "cell not in nrmac->cells[]\n");
  config_common(cell, config, scc, cell_idx);
  fill_beam_index_list(scc, config, cell);

  if (NFAPI_MODE == NFAPI_MONOLITHIC && cell == &nrmac->cells[0]) { // [TEST] use cells[1] or cells[0] to select which cells gets broadcasted
    ...
    NR_PHY_Config_t phycfg = {.Mod_id = nrmac->Mod_id, .cell_idx = cell_idx, .cfg = &cell->config};
...
bool nr_trigger_bwp_switch(gNB_MAC_INST *nrmac, uint16_t rnti, int bwp_id)
{
  ...
  if (!UE) {
    ...
    nr_mac_trigger_reconfiguration(nrmac, UE->cell, UE, bwp_id, false);
    success = true;
  }
  NR_SCHED_UNLOCK(&nrmac->sched_lock);
  return success;
