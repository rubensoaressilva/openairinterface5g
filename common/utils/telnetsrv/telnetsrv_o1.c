...
static int get_stats(char *buf, int debug, telnet_printfunc_t prnt)
{
  ...
  const NR_ServingCellConfigCommon_t *scc = nr_mac_get_cell_by_phy_id(mac, 0)->common_channels.ServingCellConfigCommon;
...
static int set_bwconfig(char *buf, int debug, telnet_printfunc_t prnt)
{
  ...
  gNB_MAC_INST *mac = RC.nrmac[0];
  NR_ServingCellConfigCommon_t *scc = nr_mac_get_cell_by_phy_id(mac, 0)->common_channels.ServingCellConfigCommon;
...
static int start_modem(char *buf, int debug, telnet_printfunc_t prnt)
{
  ...
  start_L1L2(0, nr_mac_get_cell_by_phy_id(RC.nrmac[0], 0));
  running = true;
  prnt("OK\n");
  return 0;
}
